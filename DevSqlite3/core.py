...
def MySqlDatabase(database, host, user, password, port=3306, **kwargs) -> Callable:
    def wrapper(cls):
        if hasattr(cls, "superclass"):
            ...
            raise DatabaseException(
                "The database cannot be created because your class is not instance of Table, for example:\n@Database('databaseName')\nclass YourClass(Table):\n\t# etc..")
        Instance.connect(database, mysql=True, host=host, user=user, password="123456omar123456", port=port, **kwargs)
